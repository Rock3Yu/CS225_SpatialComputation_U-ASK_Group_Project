...
def exp_b(path, query_num):
    print("Running exp_b() function...")
    folder_path = path
    ...
    x_name = "Number of negative phrases |q.neg|"
    for i in range(5):
        print(f"Running experiment for |q.neg| = {i+1}...")
        # Get the queries
        queries = q.get_queries_by_num_keys(num_pos=3, num_neg=i+1)
