...
def exp_e(path, query_num):
    print("Running exp_e() function...")
    folder_path = path
    ...
    x_labels = [5, 10, 50, 100, 500, 1000, 5000, 10000]  # aka k_list
    x_name = "k (in kNN)"
    for k in x_labels:
        ...
        for query in queries:
            res += [POWER(quadtree, inverted_index, *query, k=k)]
        ...
        for group in queries_groups_20:
            res += POWER_batch(quadtree, inverted_index, group, k=k)
